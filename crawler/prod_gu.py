...
from global_vars import gu_base_link


def get_gu_prod(driver):
    ...
    man_index_res = requests.get(
        gu_base_link + 'search?qbrand=20&qclv1=001', headers=headers
    )
    ...
    for page in range(man_prod_page):
        ...
        man_page_res = requests.get(
            gu_base_link + 'search?qbrand=20&qclv1=001&qstart=' + str(man_page_index),
            headers=headers
        )
        man_page_bs = BeautifulSoup(man_page_res.text, "lxml")
        for category in man_page_bs.select('#blkMainItemList .unit .info .name a'):
            category_url = category['href']
            if gu_base_link + 'goods/' in category_url:
                man_prod_url_set.add(category_url)
    # 取女生所有產品
    woman_prod_url_set = set()
    woman_index_res = requests.get(
        gu_base_link + 'search?qbrand=20&qclv1=002', headers=headers
    )
    woman_index_bs = BeautifulSoup(woman_index_res.text, "lxml")
    woman_prod_amount = int(
        woman_index_bs.select('#content .blkPaginationTop tr th')[0].text.replace(
            '搜尋結果：', '').replace('件', ''))
    ...
    for page in range(woman_prod_page):
        ...
        woman_page_res = requests.get(
            gu_base_link + 'search?qbrand=20&qclv1=002&qstart=' + str(woman_page_index),
            headers=headers
        )
