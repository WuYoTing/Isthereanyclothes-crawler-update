...
def get_prod_info(prod_url, driver, table_name):
    ...
    try:
        ...
        # 未處理千元小數點問題
        prod_price = page.select('#content #prodInfo #basic #price')[0].text.replace(
            'NT$', '').replace(',', '')
        prod_number = page.select('#content #prodInfo #basic .number')[0].text.replace(
            '商品編號', '')
        ...
        prod_main_image_url = \
            page.select('#content #secondary #prodMainImg #prodImgDefault')[0].img['src'].replace(
        ...
        if prod_is_limitedtime == "display: none;":
            ...
            prod_limited_price_text = page.select(
                '#content #prodInfo #goodsIconList #limitedTime #limitedPriceText'
            )[0].text.replace('止期間限定特價中', '')
