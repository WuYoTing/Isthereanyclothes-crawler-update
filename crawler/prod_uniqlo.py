...
from global_vars import uniqlo_base_link


def get_uniqlo_prod(driver):
    ...
    man_index_res = requests.get(
        uniqlo_base_link + 'search?qclv1=001', headers=headers
    )
    ...
    for page in range(woman_prod_page):
        ...
        woman_page_res = requests.get(
            uniqlo_base_link + 'search?qclv1=002&qstart=' + str(woman_page_index),
            headers=headers
        )
